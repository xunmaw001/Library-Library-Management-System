  ...
  <div>
    ...
    >  
     <el-row>
         ...
         <el-col :span="12">
             <el-form-item v-if="flag=='yonghu'"  label='系别' prop="xibieTypes">
                 <el-select v-model="ruleForm.xibieTypes"  placeholder='请选择系别'>
                     <el-option
                             v-for="(item,index) in xibieTypesOptions"
         ...
         <el-col :span="12">
           <el-form-item v-if="flag=='yonghu'"  label='图书证号' prop="yonghuUuidNumber">
               <el-input v-model="ruleForm.yonghuUuidNumber"  placeholder='图书证号' clearable></el-input>
           </el-form-item>
         </el-col>
...
export default {
  ...
  mounted() {
   ...
   this.$http({
       url: `dictionary/page?page=1&limit=100&sort=&order=&dicCode=xibie_types`,
       method: "get"
   }).then(({ data }) => {
       if (data && data.code === 0) {
          this.xibieTypesOptions = data.data.list;
      } else {
          this.$message.error(data.msg);
      }
    });
  ...
  methods: {
    ...
    onUpdateHandler() {
                         ...
                         if((!this.ruleForm.xibieTypes)&& 'yonghu'==this.flag){
                             this.$message.error('系别不能为空');
                             return
                         }
